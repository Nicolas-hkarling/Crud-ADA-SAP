...
const addStudent = (req: Request, res: Response) => {
    ...
    if (token == "Bearer 12345") {
        ...
        let sql = `INSERT INTO students(name,birthdate,address, shift, year, room) VALUES ("${student.name}","${student.birthdate}","${student.address}", "${student.shift}", "${student.year}", "${roomToUppercase}")`;

        if (student.name && student.shift && student.year && student.room && student.address && student.birthdate) {
            db.run(sql,
                (error: Error) => {
                    if (error) {
